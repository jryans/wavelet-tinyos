function []=motercoeffs(coords)

% usage:    []=motercoeffs(coords)
%
...
for j=max(scales):-1:1
    ...
    for k=1:length(predj)
        nodek=predj(k);        
        pneighbsk=predneighbs{predj(k)};
        upneighbsk=upneighbs{predj(k)};     
        fprintf \t; disp(['node ' num2str(nodek)]);
        fprintf \t; fprintf \t;   
        disp(['neighbors : ' num2str(pneighbsk)]);
        fprintf \t; fprintf \t;   
        disp(['coefficients : ' num2str(predfilts{predj(k)})]);
        upj(upneighbsk)=1;
